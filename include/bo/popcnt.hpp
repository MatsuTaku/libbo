...
constexpr uint8_t popcnt_u8(uint8_t x) {
  return kLtCnt[x];
}

constexpr uint16_t popcnt_u16(uint16_t x) {
  return kLtCnt[(x>>8)&0xFF] + kLtCnt[x&0xFF];
}

constexpr uint32_t popcnt_u32_constexpr(uint32_t x) {
  x = x - ((x >> 1) & 0x55555555u);
...
#else
  return popcnt_u32_constexpr(x);
#endif
}

constexpr uint64_t popcnt_each8_u64(uint64_t x) {
  x = x - ((x >> 1) & 0x5555555555555555ull);
  x = (x & 0x3333333333333333ull) + ((x >> 2) & 0x3333333333333333ull);
  return (x + (x >> 4)) & 0x0F0F0F0F0F0F0F0Full;
}

constexpr uint64_t popcnt_each16_u64(uint64_t x) {
  return ((popcnt_each8_u64(x) * 0x0101) >> 8) & 0x001F001F001F001F;
}

constexpr uint64_t popcnt_each32_u64(uint64_t x) {
  return ((popcnt_each8_u64(x) * 0x01010101) >> 24) & 0x0000003F0000003F;
}

constexpr uint64_t popcnt_u64_constexpr(uint64_t x) {
